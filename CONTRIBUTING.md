...
- `main` only receives commits at release points, as a merge — never
  direct development work. The pre-versioning setup commits
  on `main` (before any branch or tag existed) are the sole exception;
  from the first `0.1` branch onward, this rule is in force.
...
Every commit uses one of exactly four prefixes:
...
7. Update `CHANGELOG.md` (once one exists) and, if this bump is worth
